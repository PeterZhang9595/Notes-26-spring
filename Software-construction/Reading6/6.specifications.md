...
error不建议捕获，直接让系统终止进程，避免程序卡死；exception建议捕获并且处理，让程序可以正常运行下去。
```java
...
public class ExceptionExample {
    public static void main(String[] args) {
        try {
            // 故意读一个不存在的文件
            FileReader file = new FileReader("不存在的路径:/abc.txt");
        } catch (IOException e) {
            // 这里捕获的是 Exception，程序能继续往下跑
            System.out.println("文件没找到，我准备创建一个新文件替代...");
            System.out.println("捕获的异常类型：" + e.getClass().getSimpleName());
        }
        System.out.println("程序继续执行完毕！");
    }
}
```
![[Pasted image 20260412085934.png]]
...
```
![[Pasted image 20260412090120.png]]
- 不要滥用 `exceptions`
