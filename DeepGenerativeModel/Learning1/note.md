...
以下是图片中内容的 Markdown 格式文本：

---

# 2. 参数估计：EM 算法

GMM 的参数 $\boldsymbol{\theta} = \{\pi_k, \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\}_{k=1}^K$ 通常通过**期望最大化 (Expectation-Maximization, EM)** 算法从数据中学习。假设有 $N$ 个观测样本 $\{\mathbf{x}_i\}_{i=1}^N$，每个样本对应一个隐变量 $z_i$ 表示它来自哪个高斯分量。

### E 步（计算责任度）

用当前参数计算每个样本属于第 $k$ 个分量的后验概率（责任度）：
...
反复迭代直到对数似然收敛。在图像处理中，为了加速或适应实时性，常采用在线更新（如背景建模）或仅进行少数几次迭代（如图分割之前的 GMM 拟合）。
---
...
GMM
我们使用学习到的GMM模型对图像块patch进行预测，然后把带有噪声的图像往学习到的高斯空间上投影。

在 EPLL 的语境下，所谓“投影”其实是一个**最大后验概率估计（MAP Estimation）**的过程。

当我们说把一个“脏”的图像块投影到 GMM 空间时，实际上是在解决这样一个问题：**已知一个带噪声或退化的块 $y$，在 GMM 先验的约束下，最可能的清晰块 $x$ 是什么？**

这个过程在数学上分为两步：

### 第一步：选择最匹配的“类”（Component Selection）

GMM 包含很多个高斯分量（比如 200 个），每个分量代表一种典型的图像特征（如垂直边缘、水平边缘、平滑区域等）。

对于当前的图像块 $y$，我们计算它属于每一个高斯分量的概率。为了简化计算，EPLL 通常采用**硬分配（Hard Assignment）**：
1. 计算 $y$ 在每个高斯分量 $( \mu_k, \Sigma_k )$ 下的似然。
2. 选出概率最大的那个分量 $k^*$。这就好比在问：“这个块看起来最像哪种模式？”

### 第二步：线性滤波（The actual "Projection"）

一旦选定了第 $k^*$ 个高斯分量，我们就假设这个块服从该分量的正态分布 $\mathcal{N}(\mu_{k^*}, \Sigma_{k^*})$。

此时，我们要找的清晰块 $x$ 应该是下面这个能量函数的最小值：
$$\min_{x} \frac{1}{2\sigma^2} \|x - y\|^2 + \frac{1}{2}(x - \mu_{k^*})^T \Sigma_{k^*}^{-1} (x - \mu_{k^*})$$

*   第一项：要求 $x$ 不要偏离观测到的 $y$ 太远（保真项）。
*   第二项：要求 $x$ 符合该高斯分量的统计特性（先验项）。

这个二次型优化问题有一个**解析解**（即闭式解），这个解就是所谓的“投影”结果：
$$x = (\Sigma_{k^*} + \sigma^2 I)^{-1} (\Sigma_{k^*} \mu_{k^*} + \sigma^2 y)$$
或者变换成更常见的**维纳滤波（Wiener Filtering）**形式：
$$x = \mu_{k^*} + \Sigma_{k^*} (\Sigma_{k^*} + \sigma^2 I)^{-1} (y - \mu_{k^*})$$

----------------------------------------

这一项 $\frac{1}{2}(x - \mu_{k^*})^T \Sigma_{k^*}^{-1} (x - \mu_{k^*})$ 是高斯分布的**负对数似然（Negative Log-Likelihood）**的核心部分。
...
### 1. 数学来源：它是高斯概率的“化身”
多元高斯分布的概率密度函数公式是：
$$P(x) = \text{常数} \cdot \exp\left( -\frac{1}{2}(x - \mu)^T \Sigma^{-1} (x - \mu) \right)$$
在优化问题中，我们想让概率 $P(x)$ 最大，等价于让它的负对数最小。取完对数后，指数位置的这一串东西就掉下来了，变成了我们要最小化的**能量函数**。

### 2. 几何意义：马氏距离（Mahalanobis Distance）
这一项实际上是 $x$ 到 均值 $\mu_{k^*}$ 的**马氏距离**的平方。
*   **普通欧氏距离** $\|x - \mu\|^2$：认为所有方向（所有像素）的权重都是一样的，画出来是一个正圆形的等高线。
*   **马氏距离**：它考虑了数据的**分布形状**（由协方差矩阵 $\Sigma$ 决定）。

**直观理解：**
想象第 $k^*$ 个高斯分量代表的是“垂直边缘”。在自然图像中，垂直边缘的像素点之间有很强的相关性。
*   如果 $x$ 的变化符合垂直边缘的特征，哪怕它偏离均值稍微远一点，$\Sigma^{-1}$ 也会给它一个较小的权重，算出来的“代价”较小。
*   如果 $x$ 出现了一些杂乱的、不符合垂直边缘规律的像素（比如噪声），即使数值偏差不大，$\Sigma^{-1}$ 也会放大这个偏差，算出来的“代价”极大。

### 3. 物理作用：各向异性的“弹簧”
你可以把这一项想象成一组连接 $x$ 和 模板 $\mu_{k^*}$ 的**弹簧**：
*   **$\Sigma$ (协方差)** 告诉我们哪些方向是允许波动的。
*   **$\Sigma^{-1}$ (精度矩阵)** 则代表了弹簧的**刚度（劲度系数）**。

在 $\Sigma$ 特征值大的方向（即图像的主要特征方向，如边缘走向），弹簧很松，允许 $x$ 保留较多的原始信息；
在 $\Sigma$ 特征值小的方向（即通常认为是噪声的方向），弹簧非常硬，会强迫 $x$ 必须缩回到均值 $\mu_{k^*}$ 附近。
